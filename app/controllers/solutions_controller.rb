class SolutionsController < ApplicationController
  ...
  def create
    ...
    @solution = Solution.new(total_score: score, test_id: params[:solution][:test_id])
    ...
    if answered_questions_amount == test_questions_amount
      if @solution.save
        ...
        achieved_score = scores.map { |scores| scores.sum }.sum
        progress = achieved_score.to_f / max_score.to_f * 100.0
      else
    ...
    redirect_to live_test_path(@test, questions: correct_questions, score: "Your score is #{@test.solutions.last.total_score}(#{progress}%)")
  end
